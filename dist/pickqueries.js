...
const SearchBySalesOrder = (sonumber, inventoryOrgId) => {
...
*/
const SourceSubInventoryDetails = (inventoryOrgId, transactionTempId) => {
...
const GetLinesCountBasedOnSO = (inventoryOrgId, soNumber, soLineId) => {
 ...
 * @returns
 */
const GetSoLinesDetails = (inventoryOrgId, soNumber, soLineId, transactionTempId, deliveryId) => {
 ...
 * @returns
 */
const SearchPickDetails = (inventoryOrgId, deliveryId, itemDesc) => {
    return `SELECT  
 ...
 * @returns
 */
const DeliveryDetails = (deliveryId, inventoryOrgId, username) => {
    return `SELECT
  wv.delivery_id,
  mv.request_number,
 ...
 * @returns
 */
const UpdateLockDeliveryQuery = (deliveryId, username) => {
...
const UpdateUnLockDeliveryQuery = (deliveryId, username) => {
    return `UPDATE wsh_new_deliveries
 ...
 * @returns
 */
const GetSavedSOLineDetails = (mvOrderLineId, transactionTempId) => {
    return `SELECT
...
const GetSerialNumbers = (inventoryOrgId, soNumber, soLineId, transactionTempId) => {
    return `SELECT 
  --	  MMTT.TRANSACTION_TEMP_ID,
 ...
 */
const IsSerialNumberScanned = (transactionTempId, serialNo) => {
    return `select case
 ...
 * @returns
 */
const SaveSerialNumbers = (transactionTempId, userId, serialNum, assetNumber, transactionHeaderId) => {
 ...
 * @returns
 */
const DeleteAllSerialNumbers = (transactionTempId) => {
 ...
 * @returns
 */
const DeleteSerialNumber = (transactionTempId, serialNum) => {
 ...
 * @returns
 */
const DropAllItemsToSameLocation = (userid, toSubInv, inventoryOrgId, deliveryId, destPallet, destCage, destTote, pickedQty) => {
    return `update mtl_material_transactions_temp
 ...
 * @returns
 */
const SaveSOLinePickRelease = (userid, sourceSubInv, toSubInv, inventoryOrgId, srcPallet, srcCage, srcTote, destPallet, destCage, destTote, pickedQty, mvOrderLineId, exceptionValue, status, transactionTempId) => {
    return `update mtl_material_transactions_temp
...
const ValidateSerialNo = (subInventory, inventoryOrgId, inventoryItemId, scanValue, sourceLocatorId) => {
    // let appendQuery;
    // if (scanType === "A") {
 ...
 * @param {*} tote
 */
const ValidateOnhandQty = (inventoryOrgId, subInventoryCode, inventoryItemId, pallet, cage, tote, reqQty) => {
    return `SELECT 
          --SUBINVENTORY_CODE,
...
const LocatorAvailable = ({ destPallet, destCage, destTote, inventoryOrgId, }) => {
    return `
  SELECT  inventory_location_id
                  FROM MTL_ITEM_LOCATIONS where segment1
                       ...
                       || '.'
                       || segment3 = (
                  '${destPallet}'||'.'||nvl(${destCage ? `'${destCage}'` : null},'0')||'.'||nvl(${destTote ? `'${destTote}'` : null},'0')) and organization_id = ${inventoryOrgId}
  `;
};
const CreateLocator = ({ inventoryOrgId, deliveryId, orderHeaderId, lineId, userId, subInventoryCode, pallet, cage, tote, }) => {
    return `
  DECLARE
     l_locator number;
     l_error varchar2(2500);
  BEGIN
    XXMB_CREATE_STOCK_LOCATOR_PROC (${inventoryOrgId},${deliveryId},${orderHeaderId},${lineId},${userId},'${subInventoryCode}','${pallet}',${cage ? `'${cage}'` : `'0'`},${tote ? `'${tote}'` : `'0'`},l_locator,l_error);
 ...
 * @returns
 */
const IsSourceLocatorIdExists = (pallet, cage, tote, inventoryOrgId) => {
